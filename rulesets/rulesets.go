...
func GetAllOWASPRules() map[string]*model.Rule {
	...
	rules[OwaspNoNumericIDs] = GetOWASPRuleNoNumericIDs()
	rules[OwaspNoHttpBasic] = GetOWASPRuleNoHttpBasic()
	rules[OwaspNoAPIKeysInURL] = GetOWASPRuleNoAPIKeysInURL()
	rules[OwaspNoCredentialsInURL] = GetOWASPRuleNoCredentialsInURL()
	rules[OwaspAuthInsecureSchemes] = GetOWASPRuleAuthInsecureSchemes()
	rules[OwaspJWTBestPractices] = GetOWASPRuleJWTBestPractices()
	rules[OwaspProtectionGlobalUnsafe] = GetOWASPRuleProtectionGlobalUnsafe()
	rules[OwaspProtectionGlobalUnsafeStrict] = GetOWASPRuleProtectionGlobalUnsafeStrict()
	rules[OwaspProtectionGlobalSafe] = GetOWASPRuleProtectionGlobalSafe()
	rules[OwaspDefineErrorValidation] = GetOWASPRuleDefineErrorValidation()
	rules[OwaspDefineErrorResponses401] = GetOWASPRuleDefineErrorResponses401()
	rules[OwaspDefineErrorResponses500] = GetOWASPRuleDefineErrorResponses500()
	rules[OwaspRateLimit] = GetOWASPRuleRateLimit()
	rules[OwaspRateLimitRetryAfter] = GetOWASPRuleRateLimitRetryAfter()
	rules[OwaspDefineErrorResponses429] = GetOWASPRuleDefineErrorResponses429()
	rules[OwaspArrayLimit] = GetOWASPRuleArrayLimit()
	rules[OwaspStringLimit] = GetOWASPRuleStringLimit()
	rules[OwaspStringRestricted] = GetOWASPRuleStringRestricted()
	rules[OwaspIntegerLimit] = GetOWASPRuleIntegerLimit()
	rules[OwaspIntegerLimitLegacy] = GetOWASPRuleIntegerLimitLegacy()
	rules[OwaspIntegerFormat] = GetOWASPRuleIntegerFormat()
	rules[OwaspNoAdditionalProperties] = GetOWASPRuleNoAdditionalProperties()
	rules[OwaspConstrainedAdditionalProperties] = GetOWASPRuleConstrainedAdditionalProperties()
	rules[OwaspSecurityHostsHttpsOAS2] = GetOWASPRuleSecurityHostsHttpsOAS2()
	rules[OwaspSecurityHostsHttpsOAS3] = GetOWASPRuleSecurityHostsHttpsOAS3()
	return rules
}
...
// GenerateOWASPRuleSet generates a default ruleset for OWASP
func GenerateOWASPRuleSet() *RuleSet {
	set := &RuleSet{
		DocumentationURI: "https://blog.stoplight.io/spectral-owasp-api-security-ruleset",
		Rules:            GetAllOWASPRules(),
		Description:      "Every owasp rule from the Spectral OWASP Ruleset",
	}
	return set
}
