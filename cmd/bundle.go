...
func GetBundleCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: Bundling had errors: %s\n\n", err.Error())
				if bundled == nil {
					return err
				}
			}
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: Unable to write bundled file: '%s': %s\n\n", args[1], err.Error())
				return err
			}

			fmt.Printf("Success: Bundled OpenAPI document written to '%s'\n\n", args[1])
