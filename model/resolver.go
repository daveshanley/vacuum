...
// ResolveDocument will resolve all $ref schema nodes. Will resolve local, file based and remote nodes.
func ResolveDocument(rootNode *yaml.Node) (*yaml.Node, error) {
	...
	for _, result := range results {
		...
		if refKeyNode != nil && refValueNode != nil {
			if determineReferenceResolveType(refValueNode.Value) == localResolve {
				refResolved, _ := lookupLocalReference(refValueNode.Value, &resolvedRoot)
				if refResolved != nil {
			...
			if determineReferenceResolveType(refValueNode.Value) == httpResolve {
				refResolved, _ := lookupRemoteReference(refValueNode.Value)
				if refResolved != nil {
					refKeyNode.Content = refResolved.Content
				}
				continue
			...
			if determineReferenceResolveType(refValueNode.Value) == fileResolve {
				refResolved, _ := lookupFileReference(refValueNode.Value)
				if refResolved != nil {
					refKeyNode.Content = refResolved.Content
				}
				continue
...
func determineReferenceResolveType(ref string) int {
	...
	if strings.Contains(ref, ".json") || strings.Contains(ref, ".yaml") || strings.Contains(ref, ".yml") {
		return fileResolve
	}
	return -1
}
...
func lookupFileReference(ref string) (*yaml.Node, error) {
	...
	if seenRemoteSources[uri[0]] != nil {
		...
		body, err := ioutil.ReadFile(uri[0])
		if err != nil {
			return nil, err
		}
	...
	if parsedRemoteDocument == nil {
		return nil, fmt.Errorf("unable to parse file reference: '%s'", uri[0])
	}
