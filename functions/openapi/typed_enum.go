...
func (te TypedEnum) RunRule(nodes []*yaml.Node, context model.RuleFunctionContext) []model.RuleFunctionResult {
	...
	for j, enumNode := range nodes {
		...
		if utils.IsNodeMap(enumNode) {

			for x, enumContentItem := range enumNode.Content {
				if utils.IsNodeStringValue(enumContentItem) {
					if enumContentItem.Value == "type" {
						enumType = enumNode.Content[x+1].Value
					}
					if enumContentItem.Value == "enum" {
						enumDataNode = enumNode.Content[x+1]
					}
				}
			}
